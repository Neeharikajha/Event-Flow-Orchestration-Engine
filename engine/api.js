...
export async function init(config = {}) {
  if (!engine) {
    engine = new ProcessusEngine(config);
...
// Execute workflow (NEW: with queue/retry support)
export async function executeAsync(workflow, options = {}) {
  if (!engine) {
    await init();
  }
  return await engine.executeWorkflow(workflow, options);
}

// Execute with callback
export function execute(workflow, callback, options = {}) {
  executeAsync(workflow, options)
    .then(result => callback(null, result))
    .catch(err => callback(err));
}

// Execute with retry
export async function executeWithRetryAsync(workflow, fallback = null) {
  if (!engine) {
    await init();
  }
...
export async function queueWorkflowAsync(workflow, options = {}) {
  if (!engine) {
    await init({ useQueue: true });
  }
  return await engine.executeWorkflow(workflow, {
    ...options,
    useQueue: true
...
// Queue workflow with callback
export function queueWorkflow(workflow, options, callback) {
  queueWorkflowAsync(workflow, options)
    .then(result => callback(null, result))
    .catch(err => callback(err));
}

// Execute bulk workflows
export async function executeBulkAsync(workflows, options = {}) {
  if (!engine) {
    await init();
  }
  return await engine.executeBulk(workflows, options);
}

// Execute bulk with callback
export function executeBulk(workflows, options, callback) {
  executeBulkAsync(workflows, options)
    .then(results => callback(null, results))
    .catch(err => callback(err));
}

// Register fallback handler
export function registerFallback(workflowName, fallbackFn) {
  if (!engine) {
    throw new Error('Engine not initialized. Call init() first.');
  }
  engine.registerFallback(workflowName, fallbackFn);
}

// Get workflow status
export async function getWorkflowStatusAsync(workflowIdOrJobId, isJobId = false) {
  if (!engine) {
    await init();
  }
  return await engine.getWorkflowStatus(workflowIdOrJobId, isJobId);
}

// Get workflow status with callback
export function getWorkflowStatus(workflowIdOrJobId, isJobId, callback) {
  if (typeof isJobId === 'function') {
    callback = isJobId;
    isJobId = false;
  }
...
export async function retryFailedAsync(limit = 100) {
  if (!engine) {
    await init();
  }
  return await engine.retryFailed(limit);
}

// Cleanup old jobs
export async function cleanupAsync(graceMs = 3600000) {
  if (!engine) {
    throw new Error('Engine not initialized');
  }
  await engine.cleanup(graceMs);
}
...
export const updateWorkflow = execute;
export const deleteAll = cleanupAsync;
